...
export interface PaseoDaemonConfig {
  ...
  // SLP-PATCH(native-tools-optin): native host tools are not MCP; this governs
...
export async function createPaseoDaemon(
  ...
  // SLP-PATCH(native-tools-optin): was config.mcpInjectIntoAgents
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onListening = () => {
          ...
          const logAndResolve = async () => {
            ...
            // SLP-PATCH(native-tools-optin): the native tool channel follows
            // mcp.nativeAgentTools, never injectIntoAgents — this room injects
            // its MCP wiring itself (daemon.mcp.injectIntoAgents is false in
            // production) and still needs native tools governable on their own;
            // upstream's coupling would have switched every seat's Paseo tools
            // off with injection.
            agentManager.setPaseoToolsEnabled(config.mcpNativeAgentTools !== false);
